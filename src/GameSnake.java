 ...
 * @version 31.10.21
 *
 * Добавленные изменения: если змейка съедает желтый корм, то увеличивается на 2, ядовитый корм исчезает
 * Если змейке задается одно и то же направление дважды, она ускоряется
 */
...
public class GameSnake extends JFrame {
    ...
    final static Color SNAKE_COLOR = Color.black;
    final static Color FOOD_COLOR = Color.green;
    final static Color POISON_COLOR = Color.red;
    final static Color SUPERFOOD_COLOR = Color.yellow;
    ...
    Poison poison;                   // declare a poison
    SuperFood superFood;
    ...
    private void game() {            // method containing game cycle
        ...
        superFood = new SuperFood(snake);
        snake.setSuperFood(superFood);

        poison = new Poison(-1,-1,snake);  // poison object
        poison.setFood(food);
        poison.setSuperFood(superFood);
        ...
        while (!gameOver) {          // game cycle while NOT gameOver
            ...
            if (food.isEaten() || superFood.isEaten()) {    // if the snake ate the food
                food.appear();       //   show food in new place
                poison.plus();        //   add new poison point
                superFood.appear();
